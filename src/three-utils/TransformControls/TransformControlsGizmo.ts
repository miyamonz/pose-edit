...
const setupGizmo = (gizmoMap: any): Object3D => {
  ...
  for (let name in gizmoMap) {
    ...
    for (let i = gizmoMap[name].length; i--; ) {
      const item = gizmoMap[name][i];
      const [o, position, rotation, scale, tag] = item;
...
export class TransformControlsGizmo extends Object3D {
  ...
  constructor() {
    ...
    // たぶんmodeごとに分割するといい感じになる
    // for文内をmodeごとに関数分けていくと勝手にいい感じになりそう
  ...
  public updateMatrixWorld = (): void => {
    ...
    // handleはsetupGizmoで作られたobjectのことになる
    ...
    for (let handle of handles) {
      this.insideFor(handle, quaternion);
    }
  ...
  private insideFor(
    ...
    // insideFor内でのthis.modeへの依存はここだけっぽい
